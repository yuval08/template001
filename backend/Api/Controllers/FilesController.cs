...
public class FilesController : ControllerBase
{
    ...
    public FilesController(IFileStore fileStore, ILogger<FilesController> logger)
    {
    ...
    public async Task<ActionResult<object>> UploadFile(IFormFile file)
    {
        try
        {
            ...
            var fileName = await _fileStore.SaveFileAsync(stream, file.FileName, file.ContentType);

            _logger.LogInformation("File uploaded successfully: {FileName} -> {StoredFileName}", 
    ...
    public async Task<ActionResult> DownloadFile(string fileName)
    {
        try
        {
            ...
            var exists = await _fileStore.FileExistsAsync(fileName);
            if (!exists)
                return NotFound("File not found");

            var fileStream = await _fileStore.GetFileAsync(fileName);
    ...
    public async Task<ActionResult<object>> GetFileUrl(string fileName, [FromQuery] int expiryHours = 1)
    {
        try
        {
            ...
            var url = await _fileStore.GetFileUrlAsync(fileName, expiry);
    ...
    public async Task<ActionResult> DeleteFile(string fileName)
    {
        try
        {
            ...
            var deleted = await _fileStore.DeleteFileAsync(fileName);
